...
## Outcome / deviations from this plan

Written at the end of Phase 11. **The plan body above is left as-is on purpose** — it is the
historical record of what was intended. This section is what actually happened. Where the two
disagree, this section is correct.

All eleven phases landed. `yarn typecheck`, `yarn test` (99 tests), `yarn build` and `make start`
are green; `yarn lint` is at a pre-existing baseline this refactor did not attempt to clear (see 7).

| #   | Deviation                                                                                                                                             | Why                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| --- | ----------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | **The layering is inverted: `shared → nothing`, `types → shared`** — not `types → shared` as the §2 arrow `types → shared → {ui, core} → data` reads. | `types` needs `Time`/`DeltaTime`/`TimeRange` for `TPassage`, `TChapter`, `TTimeTrigger` and `THappening`, so it has to sit _above_ `shared`. Knock-on: `parsePassageId` went to **`core`**, not `shared` as §3's table says — it imports `@story/types`, and that was `shared`'s only edge back to `types`. Final layering, as enforced by lint: `shared → nothing`; `types → shared`; `ui → shared, types`; `core → shared, types, data`; `data → shared, types, core`; services → any package, never another service.                               |
| 2   | **`TWorldState` stayed in `data/`**, contrary to §3 ("does **not** stay here — it moves to `core/`").                                                 | It imports eight `data/` modules (locations, chapters, characters, sideCharacters). Hosting it in `core` would have made `core → data` a structural edge instead of a wart. `core` imports it **type-only**. The resulting `types ⇄ data` cycle is accepted and documented: it is type-only in every instance (`types/{ids,TChapter,TCharacter,TLocation,THappening,TItem}.ts` derive their id unions from the author's real data), it vanishes at runtime, and it is confined to the author's two folders, which §2.1 already treats as one surface. |
| 3   | **`translations.ts` and the `showToast` port moved to `shared/`, not `ui/`** (§3 put both in `ui`).                                                   | `showToast` is a port, not a component, and `data` needs to be able to reach it. Putting them in `shared` keeps `core` headless and means `data` needs no `ui` dependency at all. `ui` re-exports both, so **no call site changed**.                                                                                                                                                                                                                                                                                                                  |
| 4   | `buildWorldState` was factored out of `createWorldState` — a new function §4 does not mention.                                                        | The Visualizer's `WorldStateCopy` needs a pristine world state without constructing an `Engine`, because the `Engine` constructor replays localStorage.                                                                                                                                                                                                                                                                                                                                                                                               |
| 5   | `d3`, `@types/d3`, `jlouvain`, `ngraph.forcelayout`, `ngraph.graph` were **deleted, not moved** to `Visualizer/client` as §7 directs.                 | Zero imports anywhere in the repo. They predate the refactor.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| 6   | **`public/` is gone.** Assets resolve through `data/assets/index.ts` via `import.meta.glob`, and all three vite configs set `publicDir: false`.       | §3 moves the art into `data/assets/`, which leaves `public/` empty. Side effect: this **fixed a pre-existing 404** — `hunter.png` was referenced by two passages as `image: 'hunter'` and never resolved.                                                                                                                                                                                                                                                                                                                                             |
| 7   | **ESLint had never run.**                                                                                                                             | The config declared type-aware rules (`@typescript-eslint/require-await`, `no-floating-promises`) with no `parserOptions.project`, so it crashed on the first file it linted. Fixed in Phase 2. Running it for the first time revealed a **171-error / 5-warning pre-existing baseline**, none of it caused by this refactor and none of it cleared by it. Phase 11 added the §7 boundary rule on top and it reports **zero** violations, so the baseline is unchanged.                                                                               |
| 8   | **Phase 10 found 8 latent defects in `core`/`data`.** They are **pinned by tests, not fixed** — see below.                                            | Fixing behaviour inside a refactor makes the refactor unreviewable: a diff that is supposed to be move-only stops being verifiable by "same output". The tests encode current behaviour, so the fixes become small, isolated, obviously-correct commits later.                                                                                                                                                                                                                                                                                        |
| 9   | §8 (the Visualizer server) is still deferred and unbuilt.                                                                                             | As planned. `Visualizer/client/` is nested exactly so adding it later is purely additive. The client still hardcodes `http://localhost:3123`.                                                                                                                                                                                                                                                                                                                                                                                                         |

### Recommended next work: the 8 pinned defects

In rough severity order. Each is reproduced by a test in `core/test/` or `data/test/` that
currently asserts the **wrong** behaviour, with a comment saying so — fixing the defect means
flipping the assertion.

1. `Processor.continue` runs twice per turn — the turn loop advances the processor a second time.
2. Dead `ref` after restore — `Engine`'s post-`loadWorldState` reference points at the pre-restore object, so a restored game mutates a detached state.
3. `init` copies shallowly, so nested world-state objects are shared between the initial and live state.
4. `Inventory.addItem` ignores its `charId` argument and always credits the same inventory.
5. `replaceTime` does not revive a bare `Time[]` — it only replaces a serialized `Time` found as a _property_, never as the array element it was handed. No authored type holds a `Time[]` today, so this is a latent trap rather than a live bug; it is pinned so that adding one is a caught mistake.
6. `data/chapters/village/thomas.passages/cool.transition.ts` has a dangling `nextPassageId` — `'village-thomas-'`, an id with an empty local part. It typechecks and names no registered passage.
7. `village-thomas-cool` is unreachable: no link, redirect or transition leads to it and no character starts on it. (And if something did link to it, it would immediately hit 6.)
8. `kingdom-annie-intro` and `kingdom-annie-palace` both ask for `image: 'image'` — a placeholder that was never replaced, and that `data/assets/` does not contain.

1–5 are engine bugs (`core`); 6–8 are story data (`data`) and are the class of thing the
Visualizer is eventually meant to make impossible to author.

### Also worth doing

- **Inject the register into `core`.** `Processor`/`History` import `register` and `Story`/`Inventory` import `itemInfo` directly from `@story/data`, which is the one real value-level `core → data` edge. `createWorldState` already shows the shape of the fix. Doing it would let the boundary rule in `eslint.config.js` drop `data` from `core`'s allowed list.
- **Inject a save-store port into `Engine`.** `new Engine(s)` throws `ReferenceError: localStorage is not defined` under node, which is why `MultiEngine/server` can build a world state but not construct an engine. Details in the header comment of `MultiEngine/server/src/index.ts`.
- **HMR in docker** (§10 risk 4) has not needed `usePolling`. It was deliberately not added pre-emptively; it is the documented fallback if inotify over the bind mount turns out flaky.
