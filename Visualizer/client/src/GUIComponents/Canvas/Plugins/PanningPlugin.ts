...
export class PanningPlugin extends CanvasPluginBase implements IPluginWithControls<IPanningControls> {
    ...
    private isPanKey(key: KeyCodeType): boolean {
        return (
            this.config.panKeys.up.includes(key) ||
...
class EventHandlers {
    ...
    registerAll(): void {
        ...
        eventDispatcher.registerMouseDown('start panning', config.panMouseButton, (e, sp, wp) =>
            this.plugin.handleMouseDown(e, sp, wp)
        );
        eventDispatcher.registerMouseMove('panning', (e, sp) => this.plugin.handleMouseMove(e, sp));
        eventDispatcher.registerMouseUp('stop panning', config.panMouseButton, (e, sp, wp) =>
            this.plugin.handleMouseUp(e, sp, wp)
        );

        // Register keyboard handlers for all pan keys
        const allKeys = this.collectAllKeys(config.panKeys);
        for (const key of allKeys) {
            eventDispatcher.registerKeyDown('start panning ' + key, key, (e) => this.plugin.handleKeyDown(e));
            eventDispatcher.registerKeyUp('stop panning ' + key, key, (e) => this.plugin.handleKeyUp(e));
