...
function createMementoRecursive(obj: WithMemento, context: MementoContext): MementoResult {
    if (hasCircularReference(obj, context.visitedObjects)) {
        throw new MementoError('Circular reference detected', 'createMementoRecursive');
    }

    if (context.currentDepth >= context.options.maxDepth) {
        throw new MementoError(`Maximum depth ${context.options.maxDepth} exceeded`, 'createMementoRecursive');
    }
                        ...
                        try {
                            ...
                            if (nestedResult.success && nestedResult.autoRegisteredIds.length > 0) {
                                ...
                                for (const nestedId of nestedResult.autoRegisteredIds) {
                                    if (
                                        !autoRegisteredIds.includes(nestedId) &&
                                        !additionalAutoRegistered.includes(nestedId)
                                    ) {
